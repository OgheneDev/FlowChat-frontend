...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  initializePushNotifications: async () => {
    ...
    if (!isSupported) {
      console.log('❌ [FRONTEND] Push notifications not supported in this browser (likely iOS Safari)');
      return;
    }
  ...
  setupForegroundHandler: () => {
    if (!messaging || typeof window === 'undefined') {
      console.log('⚠️ [FRONTEND] Cannot setup foreground handler - messaging not available');
      return;
    }
    ...
    try {
      ...
      console.error('💥 [FRONTEND] Error setting up foreground handler:', error);
      // Don't throw - this is non-critical
    }
  }
}));
